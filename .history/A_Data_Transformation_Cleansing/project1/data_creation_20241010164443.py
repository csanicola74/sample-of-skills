...
# Initialize Faker
fake = Faker()  # used for generating realistic fake data

# Configuration
NUM_RECORDS = 150  # number of records to generate
DUPLICATE_PROBABILITY = 0.25  # probability of duplicate records
MISSING_VALUE_PROBABILITY = 0.1  # probability of missing values
# now create the possible values for the categorical fields
...
# randomly assigns NaN to a value based on the MISSING_VALUE_PROBABILITY
...
records = []  # this loop creates unique records based on the number of records previously defined
...
# randomly select a fraction of records to duplicate based on the DUPLICATE_PROBABILITY
duplicates = df.sample(frac=DUPLICATE_PROBABILITY, random_state=None)
df = pd.concat([df, duplicates], ignore_index=True)

# Shuffle the DataFrame
# shuffles the dataframe to mix original and duplicate records
df = df.sample(frac=1).reset_index(drop=True)

# Introduce some inconsistent date formats


# randomly changes the date format to introduce inconsistency
